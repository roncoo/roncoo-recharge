...
<div class="bjui-pageHeader">
    <form id="pagerForm" data-toggle="ajaxsearch" action="${base}/admin/sysMenu/list" method="post">
        ...
        <div class="bjui-searchBar">
            <label>所属平台：</label>
            <select name="platformId" data-toggle="selectpicker" data-width="180">
            ...
            <br />
            <br />
            <a href="${base}/admin/sysMenu/add?parentId=0&platformId=${bean.platformId!}" class="btn btn-default" data-toggle="dialog" data-icon="plus" data-id="sysMenu-add" data-options="{title:'添加', height:320, width:425}">添加一级菜单 </a>
        </div>
    </form>
</div>
    ...
    <div class="clearfix">
        <div style="float:left; width:350px; height:400px;border: 2px solid #ddd; overflow:auto;">
            ...
            <ul id="privilege" class="ztree" data-toggle="ztree" data-options="{expandAll:false,onClick:'ZtreeClick',maxAddLevel:5}" data-add-hover-dom="edit" data-remove-hover-dom="edit" data-on-remove="M_NodeRemove" >
                <#list list as bean>
                    <li data-id="${bean.id}" data-pid="${bean.parentId}" data-url="${bean.menuUrl}" data-platformid="${bean.platformId}" data-targetname="${bean.targetName!}" data-menuicon="${bean.menuIcon!}" data-sort="${bean.sort!}" data-remark="${bean.remark!}">${bean.menuName}</li>
                </#list>
            </ul>
            <#else>
        ...
        <div id="ztree-detail" style="display:none; margin-left:450px; margin-top:50px;width:500px; height:240px;">
             <div class="bs-example" data-content="详细信息">
                 ...
                 <div class="form-group">
                     ...
                     <input type="hidden" name="m.platformId" id="j_menu_platformid" />
                 </div>
                 <div class="form-group">
...
function ZtreeClick(event, treeId, treeNode) {
    ...
    if (treeNode.platformid) {$('#j_menu_platformid').val(treeNode.platformid)} 
...
function Update() {
    ...
    var platformId = $('#j_menu_platformid').val()
    ...
    if(typeof(upNode.id) == "undefined"){
        var saveData='parentId=' + upNode.pId + '&menuName=' + name + '&menuUrl=' + url + '&platformId=' + platformId + '&targetName=' + targetName + '&menuIcon=' + menuIcon + '&sort=' + sort + '&remark=' + remark;
        // 添加
        $.ajax({
            type:'post',
